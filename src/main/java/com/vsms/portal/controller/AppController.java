...
public class AppController {
    ...
    public ResponseEntity<Map<String, String>> upload(@RequestParam("file") MultipartFile file,
        ...
        if (!file.isEmpty()) {
            String extension = FilenameUtils.getExtension(Objects.requireNonNull(file.getOriginalFilename()).trim().toLowerCase());
...
//                    new File(environment.getRequiredProperty("file.location.uploads") + uploadFilename));
        ...
        System.out.println(
                String.format("Receive %s from %s service %s", file.getOriginalFilename(), transactionType));

        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }

    @PostMapping(value = "/postMessage")
    public ResponseEntity<ApiResponse<?>> postSms(@RequestBody() PostMessageRequest request) throws ApiOperationException {
    ...
    @GetMapping("/messages")
    public ResponseEntity<List<ChMessages>> getMessages() {
        List<ChMessages> messages = appService.getMessages();
        if (messages.size() > 0) {
            return new ResponseEntity<List<ChMessages>>(messages, HttpStatus.OK);
        }
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }

    @GetMapping("/transactions")
    public ResponseEntity<List<TransactionsReportView>> getTransactions() {
        List<TransactionsReportView> transactions = appService.getTransactionReport();
        if (transactions.size() > 0) {
            return new ResponseEntity<List<TransactionsReportView>>(transactions, HttpStatus.OK);
        }
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
